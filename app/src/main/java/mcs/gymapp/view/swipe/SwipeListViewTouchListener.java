...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    /*mc-001*/
    //swipe su solo elemento della lista
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    boolean allowSwipe = ((EserAdapter)swipeListView.getAdapter()).isEnabled(childPosition) && swipeListView.getAdapter().getItemViewType(childPosition) >= 0;

                    if (allowSwipe && rect.contains(x, y)
          /*mc-001*/                && (openItem < 0  || openItem == childPosition )
          //   /*mc-002*/                && (!swipeListView.isInvalid(childPosition))
                            ) {
                        setParentView(child);
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!((EserAdapter)swipeListView.getAdapter()).isEnabled(downPosition) )
                    break;
